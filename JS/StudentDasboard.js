document.addEventListener('DOMContentLoaded', function() {
  ...
  const themes = {
    light: {
      icon: 'bi-moon',
      particleColor: 'rgba(80, 75, 56, 0.15)'
    },
    dark: {
      icon: 'bi-sun',
      particleColor: 'rgba(232, 227, 197, 0.15)'
    }
  };
  ...
  function initCounters() {
    const counters = [
      { id: 'quick-assignments', target: 4 },
      { id: 'quick-classes', target: 5 },
      { id: 'quick-grades', target: 88 }
    ];
    ...
    counters.forEach(counter => {
      ...
      if (element) {
        animateCounter(
          element, 
          counter.target, 
          counter.id.includes('grades') || counter.id.includes('attendance')
        );
      }
    });
  }
